...
describe(colors.red("Onion Auction testing").bgGreen, function () {
  ...
  it("Deployment of Primary Market Place Contract", async ()=>{
    ...
    [owner,  buyer1, buyer2, buyer3, buyer4, treasureWallet, operationalWallet, AtheleteAddress, FedAddress, user1, user2, user3, user4, user5 ] = await ethers.getSigners();
  ...
  it("Approving All the ERC1155 to the Market Place contract", async()=>{
  ...
  it("Creating multiple auction using different accounts", async()=>{
    await PrimaryMP.connect(buyer1).createOnionAuction([1], buyer1.address, ERC20BytesAddress.hash, [5], ["20000000000000000000"], [10]) 
    expect(await ERC1155.balanceOf(PrimaryMP.address, 1)).to.equal(5)   
    await PrimaryMP.connect(buyer2).createOnionAuction([2], buyer2.address, ERC20BytesAddress.hash, [5], [5], [10])    
